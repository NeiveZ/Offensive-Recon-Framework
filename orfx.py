...
from utils.colors import Colors, banner as ui_banner, print_section, print_status, print_table
...
def print_commands():
    ...
    print(f"\nGlobal: --json --txt --html --out PATH --silent --no-color --details")
    print("\nExamples:")
    print("  User-Agent modes: --user-agent random | --user-agent @path/to/agents.txt | --user-agent 'exact value'")
    examples = [
...
def parse_common(argv):
    common = {"json": False, "txt": False, "html": False, "out": "", "silent": False, "no_color": False, "details": False}
    cleaned = []
    i = 0
    while i < len(argv):
        item = argv[i]
        if item in ("--json", "--txt", "--html", "--silent", "--no-color", "--details"):
...
def show_module_help(command, obj):
    ...
    print("\nAliases: -d/-u/-t for TARGET, -i for INPUT, plus global report/output flags.")
...
def run_check():
    ...
    module_ok = loaded == len(COMMANDS) and not any(label.startswith("Module ") for label, _, _ in checks)
    checks.insert(1, ("Modules", f"{loaded}/{len(COMMANDS)} loaded", module_ok))
    tools = [tool for tool in ("dig", "whois", "nmap") if shutil.which(tool)]
    checks.append(("Tools", ", ".join(tools) if tools else "none detected", True))
    compiled = compileall.compile_dir(".", quiet=1)
    checks.append(("Python syntax", "PASS" if compiled else "FAIL", compiled))
    print(f"\n{Colors.BOLD}{Colors.WHITE}Environment Check{Colors.RESET}")
    print("  " + "-" * 66)
    for label, value, passed in checks:
        ok = ok and passed
        marker = f"{Colors.GREEN}[+]{Colors.RESET}" if passed else f"{Colors.RED}[-]{Colors.RESET}"
        print(f"  {marker} {Colors.WHITE}{label:<16}{Colors.RESET} {value}")
    print("  " + "-" * 66)
    print(f"  {Colors.GREEN if ok else Colors.RED}{'[+] ORFX is ready' if ok else '[-] ORFX check failed'}{Colors.RESET}\n")
    return 0 if ok else 1


def _display_option_name(name):
    labels = {
        "TARGET": "Target", "TARGETS": "Targets", "INPUT": "Input", "PROFILE": "Profile",
        "THREADS": "Threads", "TIMEOUT": "Timeout", "RETRIES": "Retries", "DELAY": "Delay",
        "WORDLIST": "Wordlist", "RESOLVE": "Resolve", "PASSIVE": "Passive", "ACTIVE": "Active",
        "HTTP": "HTTP", "USER_AGENT": "User-Agent", "PORTS": "Ports", "BANNERS": "Banners",
        "RECORDS": "Records", "SERVER": "DNS server", "SCHEME": "Scheme", "FOLLOW": "Follow redirects",
        "FALLBACK": "HTTP fallback", "VERIFY": "TLS verify", "PORT": "Port",
    }
    return labels.get(name, name.replace("_", " ").title())


def _format_option_value(name, value):
    if value in (None, ""):
        return "-"
    text = str(value)
    if name == "WORDLIST":
        return text if text not in ("auto", "default") else "bundled default"
    if name == "USER_AGENT" and text.startswith("ORFX/"):
        return "default"
    if name in {"RESOLVE", "PASSIVE", "ACTIVE", "HTTP", "BANNERS", "FOLLOW", "VERIFY", "FALLBACK"}:
        return "enabled" if text.lower() in {"true", "1", "yes", "on"} else "disabled"
    if name == "TIMEOUT":
        return f"{text}s" if text.lower() not in {"auto", "default"} and not text.endswith("s") else text
    if name == "DELAY":
        return f"{text}s" if text.lower() not in {"auto", "default"} and not text.endswith("s") else text
    return text


def print_run_config(command, obj):
    print(f"\n{Colors.BOLD}{Colors.WHITE}Recon Configuration{Colors.RESET}")
    headers = ["Parameter", "Value"]
    rows = [("Command", command)]
    preferred = [
        "TARGET", "PROFILE", "THREADS", "TIMEOUT", "RETRIES", "WORDLIST",
        "RESOLVE", "USER_AGENT", "RECORDS", "SERVER", "SCHEME",
        "FOLLOW", "FALLBACK", "VERIFY", "PORTS", "BANNERS", "PORT",
    ]
    for name in preferred:
        if name not in obj.options:
            continue
        value = obj.get_option(name)
        if name not in {"TARGET", "PROFILE", "THREADS", "TIMEOUT", "RETRIES", "WORDLIST", "RESOLVE", "USER_AGENT", "RECORDS", "SERVER", "SCHEME", "PORTS", "BANNERS", "FOLLOW", "FALLBACK", "VERIFY", "PORT"}:
            continue
        rows.append((_display_option_name(name), _format_option_value(name, value)))
    if not rows:
        rows.append(("Command", command))
    print_table(headers, rows, indent=2)
...
def run_command(command, args, common):
    ...
    if not common["silent"]:
        banner()
        print_run_config(command, obj)
        print_section(f"{command.replace('_', ' ').title()} → {Colors.CYAN}{obj.get_option('TARGET') or obj.get_option('TARGETS') or ''}{Colors.RESET}")
    try:
    ...
    if not common["silent"]:
        ...
        print_status(f"{len(rows)} structured finding(s) collected.", "ok")
        if common["details"]:
            print_rows(rows)
    else:
...
def run_full(args, common):
    ...
    while i < len(clean_args):
        ...
        if arg == "--threads":
            if i + 1 >= len(clean_args):
                print(f"{Colors.RED}--threads requires a value{Colors.RESET}")
                return 2
            threads = clean_args[i + 1]; i += 2; continue
        if arg == "--profile":
            if i + 1 >= len(clean_args):
                print(f"{Colors.RED}--profile requires a value{Colors.RESET}")
                return 2
            profile = clean_args[i + 1]; i += 2; continue
        if arg == "--timeout":
            if i + 1 >= len(clean_args):
                print(f"{Colors.RED}--timeout requires a value{Colors.RESET}")
                return 2
            timeout = clean_args[i + 1]; i += 2; continue
        if arg == "--user-agent":
            if i + 1 >= len(clean_args):
                print(f"{Colors.RED}--user-agent requires a value{Colors.RESET}")
                return 2
            user_agent = clean_args[i + 1]; i += 2; continue
        if arg == "--ports":
    ...
    if threads: sub.set_option("THREADS", threads)
    if profile: sub.set_option("PROFILE", profile)
    if timeout: sub.set_option("TIMEOUT", timeout)
    if user_agent: sub.set_option("USER_AGENT", user_agent)
    sub_result = sub.run()
    ...
    if http_enabled:
        ...
        for host in hosts[:100]:
            ...
            if user_agent: http.set_option("USER_AGENT", user_agent)
            result = http.run()
    ...
    if not common["silent"]:
        print(f"\n{Colors.BOLD}{Colors.CYAN}Recon Summary{Colors.RESET}")
