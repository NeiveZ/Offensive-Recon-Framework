...
from core.helpers import as_bool, safe_float, choose_user_agent, load_user_agents
...
class HTTPProbe(BaseModule):
    ...
    def _define_options(self):
        ...
        self._add_option("USER_AGENT", "ORFX/3.2.3 (Security Assessment)", False, "HTTP User-Agent; use random/rotate or @/path/to/agents.txt for controlled rotation")
    ...
    def _probe(self, raw):
        ...
        user_agent_value = str(self.get_option("USER_AGENT"))
        user_agent_pool = load_user_agents(user_agent_value[1:] if user_agent_value.startswith("@") else None)
        last_error = ""
        for url in candidates:
            try:
                context = ssl.create_default_context()
                user_agent = choose_user_agent(user_agent_value, user_agent_pool)
